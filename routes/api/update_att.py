...
update_att_bp = Blueprint('update_att_bp', __name__, url_prefix='/api')


@update_att_bp.route('/atualizacao/<int:id>', methods=['PUT'])
@token_required
def update_evento(id):
    ...
    try:
        ...
        if imagem:
            ...
            cur.execute(
                "UPDATE atualizacoes SET texto = %s, imagem = %s WHERE id = %s",
                (texto.strip(), image_url, id)
            )
        else:
            cur.execute(
                "UPDATE atualizacoes SET texto = %s WHERE id = %s",
                (texto.strip(), id)
            )
        ...
        conn.rollback()
        logger.error(f"Erro ao atualizar atualizacao {id} | IP: {request.remote_addr} | Erro: {str(e)}")
        return jsonify({"error": "Erro ao atualizar o atualizacao"}), 500
