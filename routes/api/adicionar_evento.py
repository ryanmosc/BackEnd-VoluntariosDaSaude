...
adicionar_evento_bp = Blueprint('adicionar_evento_bp', __name__, url_prefix='/api')

@adicionar_evento_bp.route('/eventos', methods=['POST'])
...
def add_evento():
    ...
    if imagem and imagem.mimetype in ['image/jpeg', 'image/png']:
        ...
        try:
            ...
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO eventos (texto, imagem, data_criacao) VALUES (%s, %s, CURRENT_TIMESTAMP) RETURNING id",
                (texto, image_url)
            )
